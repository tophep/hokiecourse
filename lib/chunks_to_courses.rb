module ChunksToCourses
	...
	# Creates all courses listed in the info files generated by scrape_info.rake
	def self.create_all_courses(year, term)
		DataToChunks.extract_all_chunks.each do |chunk|
			create_course_from_chunk(chunk, year, term) if FORMAT_TO_ATTRIBUTE_MAP[chunk.format]
		end
	end

	# Assumes the currently saved course listings are of open courses only
	def self.update_open_courses(year, term)
		...
		DataToChunks.extract_all_chunks.each do |chunk|
			...
			if course = Course.where(year: year, term: term, crn: crn).first
	...
	def self.create_course_from_chunk(chunk, year, term)
		attributes = extract_attributes_from_chunk(chunk)
	...
	def self.create_course(attributes)
		if course = Course.find_by(crn: attributes[:crn], year: attributes[:year], term: attributes[:term])
	...
	def self.extract_attributes_from_chunk(chunk)
		info = chunk.data
		attribute_locations = FORMAT_TO_ATTRIBUTE_MAP[chunk.format]
